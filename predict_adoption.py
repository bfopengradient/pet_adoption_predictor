...
class pets:
	...
	def run_model(data,labels):		  
        ...
        #Keep record of the predictor matrix used in the model. Save table as 'my_predictor_matrix' in the dbpets database.    
		pd.DataFrame(X_train).to_sql(name='my_predictor_matrix',con=con,if_exists='replace')
		#Save copy of the predictior matrix to a csv file, insert your file path. 
		pd.DataFrame(X_train).to_csv('....my_predictor_matrix.csv')
		...
		#If needed, output matrix for each layer.
		outputs = []
		for layer in model_1.layers:
		    keras_function = K.function([model_1.input], [layer.output])
		    outputs.append(keras_function([X_train, 1]))

		#Save copy of the output from the Embedding layer only. Save to your file path.
		np.save('.....my_embeddings.npy', np.array(outputs[0]))
  
		 
		 
		#For this exercise I am using online batch learning where weights are updated after each record is feed to algorithm. 
		#For quicker training model is stable up to a higher batch sizes of 100. 
		#Compile,fit model and evaluate on test data.  
		model_1.compile(optimizer='adam', loss='binary_crossentropy',metrics=['acc'])
		#Spliting the training data into an 80/20 split where 20% of records will be used in training to help the model learn from it's errors.		 
